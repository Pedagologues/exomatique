...
export async function get(document_id: string, access_token?: string): Promise<FrontDocument> {
	const document = await Document.findById(document_id);
...
export async function link(document_id: string, parent: string, access_token: string) {
	const document = await Document.findById(document_id);
...
export async function unlink(document_id: string, parent: string, access_token: string) {
	const document = await Document.findById(document_id);
...
export async function add_child(
	...
	if (document.children.get(name) != null) throw new ChildExist(document_id, name);

	await link(child_id, document_id, access_token);

	document.children.set(name, mongoose.Types.ObjectId.createFromHexString(child_id));
	document.updated = new Date();
		...
		if (Math.abs(doc.updated.getTime() - date.getTime()) < TIME_BEFORE_DELETION) {
			console.log(
				`Document ${doc._id} is not linked but is still in the waiting phase. Document will be removed in ${
					TIME_BEFORE_DELETION - Math.abs(doc.updated.getTime() - date.getTime())
				}ms`
			);
			return;
		}
